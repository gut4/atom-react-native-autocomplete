...
var provider = {
    ...
    getSuggestions: function getSuggestions(request) {
        ...
        return new Promise(function (resolve) {
            ...
            if (_component2.default.isAttributeValue(scopes)) {
                ...
                suggestion = _component2.default.getTagNameCompletions(line);
    ...
    onDidInsertSuggestion: function onDidInsertSuggestion(request) {
        ...
        if(request.suggestion.type == "tag") {
          var currentPosition = request.editor.getCursorBufferPosition()
          request.editor.setCursorBufferPosition([request.triggerPosition.row, request.triggerPosition.column-request.suggestion.replacementPrefix.length]);
          var firstChar = request.editor.getTextInBufferRange([
            request.editor.getCursorBufferPosition().translate([0, -1]),
            request.editor.getCursorBufferPosition()
          ]);


          if(firstChar != '<') {
            request.editor.insertText('<');
            request.editor.setCursorBufferPosition(currentPosition.translate([0, 1]));
          }
          else {
            request.editor.setCursorBufferPosition(currentPosition)
          }
        }
    },
